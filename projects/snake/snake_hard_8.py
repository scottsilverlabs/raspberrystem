...
class Snake():
    ...
    def head(self):
        return self.body[0]   # return the first point of the body
...
while True:
    ...
    if snake.has_collided_with_self():
        led_matrix.text(str(snake.length()))
        led_matrix.show()
        break
    ...
    if apple.position == snake.head():
        snake.move(growing=True)   # collision happened grow the snake
        apple = Apple(choice(all_points) - set(snake.body))
    else:
