...
# Create a set of all the points on the LED matrix
...
while True:
    ...
    if apple_position == snake[0]:
        ...
        apple_position = choice(all_points - set(snake))
        # grow the snake, ie don't remove the tail
    ...
    # check if the snake has collided with itself, and lose if we do
    if snake[0] in snake[1:]:
        # display the score and then break out of the while loop
        led_matrix.text(str(len(snake)))  # print the length of the snake
        led_matrix.show()
