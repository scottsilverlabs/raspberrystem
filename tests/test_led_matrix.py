...
class PrivateFunctions(unittest.TestCase):
    ...
    def test_valid_color(self):
        for i in range(16):
            self.assertTrue(led2.valid_color(i))
            self.assertTrue(led2.valid_color(hex(i)[2:]))
        self.assertTrue(led2.valid_color('-'))
        self.assertFalse(led2.valid_color(17))
        self.assertFalse(led2.valid_color(-1))
        # random invalid characters
        self.assertFalse(led2.valid_color('j'))
        self.assertFalse(led2.valid_color('%'))
        self.assertFalse(led2.valid_color('10'))  #TODO : should we allow none hex versions?
        
    def testconvert_color(self):
        self.assertRaises(ValueError, led2.convert_color('J')) # FIXME: syntax correct?
        for i in range(16):
            self.assertEquals(led2.convert_color(hex(i)[2:]), i)
            self.assertEquals(led2.convert_color(i), i)
...
class PrimativeFunctions(unittest.TestCase):
    ...
    def test_line(self):
        self.assertEquals(led2.line((0,0),(15,7), 1), 1)
        self.assertEquals(led2.show(), 1)
